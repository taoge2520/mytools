...
func Test(msg []byte, off int) (string, int) {
	s := make([]byte, 0, 64)

	lenmsg := len(msg)
	s = append(s, msg[off-1])
	for {
		//i := int(1)
		if off >= lenmsg {

			return string(s), lenmsg
		}
...
func UnpackRR(msg []byte, off int) (rr RR, off1 int) {
	...
	} else if h.Rrtype == TypeTXT {
		rr, off, _ = unpackTXT(h, msg, off)
...
func unpackRRslice(l int, msg []byte, off int) (dst1 []RR, off1 int) { ///continue
	var r RR
